...
class SeedDataCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (count($users) > 0) {
            ...
            // actually create the notifications now that the list exists
            foreach ($users as $user) {
                for ($i = 0; $i < 3; $i++) {
        ...
        // Seed parapharmacy products if none exist
        ...
        if (count($existingProducts) === 0) {
            ...
            for ($i = 1; $i <= 50; $i++) {
                ...
                $product->setDescription("Sample description for product {$i}.");
                // random price between 1.00 and 100.00
            ...
            $output->writeln('  - Parapharmacy products already exist, skipping...');
        }
